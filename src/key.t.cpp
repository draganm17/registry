...
TEST(Key, OperationsOnRegistry)
{
    ...
    {
        std::error_code ec;
        const key k(open_only_tag{}, TEXT("HKEY_CURRENT_USER\\SOFTWARE"), access_rights::read);

        EXPECT_TRUE(k.key_exists(TEXT("libregistry")) == true);
        EXPECT_TRUE(k.key_exists(TEXT("libregistry"), ec) == true && !ec);

        EXPECT_TRUE(k.key_exists(TEXT("non_existent")) == false);
        EXPECT_TRUE(k.key_exists(TEXT("non_existent"), ec) == false && !ec);
    }
    ...
    {
        std::error_code ec;
        const key k(open_only_tag{}, TEXT("HKEY_CURRENT_USER\\SOFTWARE\\libregistry\\read"), access_rights::query_value);

        EXPECT_TRUE(k.value_exists(TEXT("val_01")) == true);
        EXPECT_TRUE(k.value_exists(TEXT("val_01"), ec) == true && !ec);

        EXPECT_TRUE(k.value_exists(TEXT("non_existent")) == false);
        EXPECT_TRUE(k.value_exists(TEXT("non_existent"), ec) == false && !ec);
    }
    ...
    // key::read_value(string_view_type, std::error_code&)
    {
        ...
        auto v01  = k.read_value(TEXT("val_01"));
        auto v01a = k.read_value(TEXT("val_01"), ec);
        EXPECT_TRUE(!ec && v01 == v01a);
        EXPECT_TRUE(v01.type() == value_type::none && v01.data().size() == 0);

        auto v02  = k.read_value(TEXT("val_02"));
        auto v02a = k.read_value(TEXT("val_02"), ec);
        EXPECT_TRUE(!ec && v02 == v02a);
        EXPECT_TRUE(v02.type() == value_type::sz && v02.to_string() == TEXT("42"));

        auto v03  = k.read_value(TEXT("val_03"));
        auto v03a = k.read_value(TEXT("val_03"), ec);
    ...
    {
        std::error_code ec;
        key k(open_or_create_tag{}, TEXT("HKEY_CURRENT_USER\\SOFTWARE\\libregistry\\write"), access_rights::all_access);

        // create new keys (without subkeys)
        const key_path p1 = TEXT("new_key_1");
        ...
        const key_path p3 = TEXT("new_key_3\\Inner1\\Inner2");
    ...
    // key::write_value(string_view_type, const value&, std::error_code&)
    {
        ...
        key k(open_only_tag{}, 
              TEXT("HKEY_CURRENT_USER\\SOFTWARE\\libregistry\\write"),
              access_rights::set_value | access_rights::query_value);
        
        const value v01(none_value_tag{});
        k.write_value(TEXT("val_01"), v01);
        EXPECT_TRUE((k.write_value(TEXT("val_01a"), v01, ec), !ec));
        EXPECT_TRUE(k.read_value(TEXT("val_01")) == v01 && k.read_value(TEXT("val_01a")) == v01);

        const value v02(sz_value_tag{}, TEXT("42"));
        k.write_value(TEXT("val_02"), v02);
        EXPECT_TRUE((k.write_value(TEXT("val_02a"), v02, ec), !ec));
        EXPECT_TRUE(k.read_value(TEXT("val_02")) == v02 && k.read_value(TEXT("val_02a")) == v02);

        const value v03(expand_sz_value_tag{}, TEXT("42"));
        k.write_value(TEXT("val_03"), v03);
        EXPECT_TRUE((k.write_value(TEXT("val_03a"), v03, ec), !ec));
        EXPECT_TRUE(k.read_value(TEXT("val_03")) == v03 && k.read_value(TEXT("val_03a")) == v03);

        const value v04(binary_value_tag{}, bytes.data(), bytes.size());
        k.write_value(TEXT("val_04"), v04);
        EXPECT_TRUE((k.write_value(TEXT("val_04a"), v04, ec), !ec));
        EXPECT_TRUE(k.read_value(TEXT("val_04")) == v04 && k.read_value(TEXT("val_04a")) == v04);

        const value v05(dword_value_tag{}, 42);
        k.write_value(TEXT("val_05"), v05);
        EXPECT_TRUE((k.write_value(TEXT("val_05a"), v05, ec), !ec));
        EXPECT_TRUE(k.read_value(TEXT("val_05")) == v05 && k.read_value(TEXT("val_05a")) == v05);

        const value v06(dword_big_endian_value_tag{}, 42);
        k.write_value(TEXT("val_06"), v06);
        EXPECT_TRUE((k.write_value(TEXT("val_06a"), v06, ec), !ec));
        EXPECT_TRUE(k.read_value(TEXT("val_06")) == v06 && k.read_value(TEXT("val_06a")) == v06);

        const value v07(link_value_tag{}, TEXT("42"));
        k.write_value(TEXT("val_07"), v07);
        EXPECT_TRUE((k.write_value(TEXT("val_07a"), v07, ec), !ec));
        EXPECT_TRUE(k.read_value(TEXT("val_07")) == v07 && k.read_value(TEXT("val_07a")) == v07);

        const value v08(multi_sz_value_tag{}, { TEXT("42"), TEXT("42") });
        k.write_value(TEXT("val_08"), v08);
        EXPECT_TRUE((k.write_value(TEXT("val_08a"), v08, ec), !ec));
        EXPECT_TRUE(k.read_value(TEXT("val_08")) == v08 && k.read_value(TEXT("val_08a")) == v08);

        const value v09(qword_value_tag{}, 42);
        k.write_value(TEXT("val_09"), v09);
        EXPECT_TRUE((k.write_value(TEXT("val_09a"), v09, ec), !ec));
        EXPECT_TRUE(k.read_value(TEXT("val_09")) == v09 && k.read_value(TEXT("val_09a")) == v09);  
    }
    ...
    {
        std::error_code ec;
        key k(open_only_tag{}, TEXT("HKEY_CURRENT_USER\\SOFTWARE\\libregistry\\write"), access_rights::set_value | 
                                                                                        access_rights::query_value);

        ASSERT_TRUE(!k.value_exists(TEXT("non_existing")) &&
                     k.value_exists(TEXT("val_01"))       && 
                     k.value_exists(TEXT("val_02")));

        // remove an non-existing value
        EXPECT_TRUE(k.remove_value(TEXT("non_existing")) == false);
        EXPECT_TRUE(k.remove_value(TEXT("non_existing"), ec) == false && !ec);

        // remove an existing value
        EXPECT_TRUE(k.remove_value(TEXT("val_01")) == true && !k.value_exists(TEXT("val_01")));
        EXPECT_TRUE(k.remove_value(TEXT("val_02"), ec) == true && !ec && !k.value_exists(TEXT("val_02")));
    }
...
TEST(Key, Close)
{
    ...
    // close a regular key
    key k5(open_only_tag{}, TEXT("HKEY_CURRENT_USER\\SOFTWARE\\libregistry"), access_rights::read);
    k5.close();
    EXPECT_TRUE(!k5.is_open());
    //
    key k6(open_only_tag{}, TEXT("HKEY_CURRENT_USER\\SOFTWARE\\libregistry"), access_rights::read);
    k6.close(ec);
    EXPECT_TRUE(!ec && !k6.is_open());
}
