...
namespace registry
{
    ...
    /*! key_iterator is an InputIterator that iterates over the key elements of a registry key (but
    //  does not visit the subkeys). The iteration order is unspecified, except that each entry is 
    //  visited only once. If the `key_iterator` is advanced past the last entry, it becomes equal 
    //  to the default-constructed iterator, also known as the end iterator. Two end iterators are 
    //  always equal, dereferencing or incrementing the end iterator is undefined behavior. If an 
    //  entry is deleted or added to the key tree after the key iterator has been created, it is 
    ...
    */
    ...
    public:
        ...
        */
        ...
        /*! \brief
        //  Constructs the iterator with the contents of `other` using move semantics. `other` is
        //  left in a valid but unspecified state. */
        /*!
        //  @post `*this` has the original value of `other`.
        */
        ...
        /*!
        //  The address of `key` is stored in a data member. Calling any non-const member function 
        //  of `key` invalidates this iterator. \n
        //  `key` must have been opened with the `access_rights::enumerate_sub_keys` and
        ...
        */
        ...
        /*!
        //  If `path` refers to an non-existing registry key, returns the end iterator and does not
        //  report an error. \n
        //  The overload that takes `std::error_code&` parameter constructs an end iterator on error.
        //
        //  @param[in]  path - an absolute key path specifying the registry key that this iterator
        //                     iterates on.
        //
        //  @param[out] ec   - out-parameter for error reporting.
        //
    ...
    public:
        ...
        */
        ...
        /*! If an error occured, `*this` is set to an end iterator, regardless of whether any error
        //  is reported by exception or error code.
        //
        //  @pre `*this != key_iterator()`.
        */
    ...
    /*! recursive_key_iterator is an InputIterator that iterates over the key elements of a registry
    //  key, and, recursively, over the entries of all subkeys. The iteration order is unspecified, 
    //  except that each entry is visited only once. If the `recursive_key_iterator` is advanced past
    //  the last entry of the top-level registry key, it becomes equal to the default-constructed 
    //  iterator, also known as the end iterator. Two end iterators are always equal, dereferencing
    //  or incrementing the end iterator is undefined behavior. If an entry is deleted or added to 
    //  the key tree after the recursive key iterator has been created, it is unspecified whether 
    //  the change would be observed through the iterator.
    */
    ...
    public:
        ...
        */
        ...
        //  The address of `key` is stored in a data member. Calling any non-const member function
        //  of `key` invalidates this iterator. \n
        //  if `(options & key_options::skip_permission_denied) != key_options::none` and construction
        //  encounters an error indicating that permission to access `key` is denied, constructs the
        ...
        */
        ...
        //  report an error. \n
        //  if `(options & key_options::skip_permission_denied) != key_options::none` and construction
        //  encounters an error indicating that permission to access `path` is denied, constructs the 
        //  end iterator and does not report an error. \n
        //  The overload that takes  `std::error_code&` parameter constructs an end iterator on error.
        //
    ...
    public:
        ...
        */
        ...
        /*! \brief
        //  Returns the number of keys from the starting key to the currently iterated key, i.e. 
        //  the current depth of the key hierarchy. */
        /*!
        ...
        */
        ...
        /*!
        //  The options can only be supplied when constructing the key iterator. If the options 
        //  argument was not supplied, returns `key_options::none`.
        //
        //  @pre `*this != recursive_key_iterator()`.
        */
    ...
    public:
        ...
        */
        ...
        //! Advances the iterator to the next entry. 
        /*! If there are no more entries left in the currently iterated key, the iteration is resumed 
        //  over the parent key. The process is repeated if the parent key has no sibling entries that 
        //  can to be iterated on. If the parent of the key hierarchy that has been recursively iterated 
        //  on is reached (there are no candidate entries at `depth() == 0`), `*this` is set to an end 
        //  iterator. Otherwise, `*this` is iterated. \n
        //  If an error occured, `*this` is set to an end iterator, regardless of whether any error is 
        //  reported by exception or error code.
        //
        //  @pre `*this != recursive_key_iterator()`.
        */
        ...
        /*!
        //  If `depth() == 0`, set `*this` to `recursive_key_iterator()`. Otherwise, cease iteration 
        //  of the key currently being iterated over, and continue iteration over the parent key. \n
        //  If an error occured, `*this` is set to an end iterator, regardless of whether any error 
        //  is reported by exception or error code.
        //
        //  @pre `*this != recursive_key_iterator()`.
    ...
    inline key_options& operator&=(key_options& lhs, key_options rhs) noexcept
    {
        return lhs = lhs & rhs;
    }
    ...
    inline bool operator==(const key_entry& lhs, const key_entry& rhs) noexcept
    {
    ...
    inline void swap(key_entry& lhs, key_entry& rhs) noexcept
    {
        lhs.swap(rhs);
    }
    ...
    inline key_iterator end(const key_iterator&) noexcept
    {
        return key_iterator();
    }

    inline void swap(key_iterator& lhs, key_iterator& rhs) noexcept
    {
        lhs.swap(rhs);
    }
    ...
    inline recursive_key_iterator end(const recursive_key_iterator&) noexcept
    {
        return recursive_key_iterator();
    }

    inline void swap(recursive_key_iterator& lhs, recursive_key_iterator& rhs) noexcept
    {
        lhs.swap(rhs);
    }
