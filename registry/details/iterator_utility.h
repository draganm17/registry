...
namespace details {
    ...
    public:
        explicit operator bool() const noexcept { return std::visit([this](auto&& v) { return do_check(v); }, m_ptr); }

        T* operator->() const noexcept { return std::visit([this](auto&& v) { return do_get(v); }, m_ptr); }

    public:
        void swap(possibly_ptr& other) noexcept(noexcept(m_ptr.swap(other.m_ptr))) { m_ptr.swap(other.m_ptr); }

    private:
        std::variant<T*, std::remove_const_t<T>> m_ptr;
    };
    ...
    public:
        explicit operator bool() const noexcept { return std::visit([](auto&& v) { return static_cast<bool>(v); }, m_ptr); }

        T* operator->() const noexcept { return std::visit([](auto&& v) { return std::addressof(*v); }, m_ptr); }

    public:
        void swap(possibly_shared_ptr& other) noexcept { m_ptr.swap(other.m_ptr); }

    private:
        std::variant<T*, std::shared_ptr<T>> m_ptr;
    };
    ...
    public:
        possibly_shared_ptr<T> lock() const noexcept { return std::visit([this](auto&& v) { return do_lock(v); }, m_ptr); }

    public:
        void swap(possibly_weak_ptr& other) noexcept { m_ptr.swap(other.m_ptr); }

    private:
        std::variant<T*, std::weak_ptr<T>> m_ptr;
    };
