    ...
    */
    ...
    public:
        ...
        */
        ...
        /*!
        //  Unless `id == key_id::unknown` postconditions are the following:
        //  - `is_open()`.
        ...
        */
        ...
        //! Opens a registry key and associates it with `*this`.
        /*!
        //  The overload that takes `std::error_code&` parameter constructs an `key` object that  
        //  does not represent a registry key on error.
        //
        //  @post `is_open()`.
        //
        //  @post `this->rights() == rights`.
        //
        //  @param[in]  path   - an absolute key path specifying the registry key that this
        //                       function opens.
        //
        ...
        //
        //  @throw 
        //      The overload that does not take a `std::error_code&` parameter throws `registry_error`
        //      on underlying OS API errors, constructed with the first key path set to `path` and the
        //      OS error code as the error code argument. \n
        //      The overload that does take a `std::error_code&` parameter sets it to the OS API error
        //      code if an OS API call fails, and executes `ec.clear()` if no errors occur. \n
        //      `std::bad_alloc` may be thrown by both overloads if memory allocation fails.
        */
        key(open_only_tag, const key_path& path, access_rights rights, std::error_code& ec = throws());


        //! Opens or creates a registry key and associates it with `*this`.
        /*! Identical to the overload that takes `bool&` parameter, except does not report if the 
        //  key was created or not.
        */
        key(open_or_create_tag, const key_path& path, access_rights rights, std::error_code& ec = throws());
        ...
        //
        //  @param[out] was_created - receives `true` if the key was created and `false` if it
        //                            already existed.
        //
        //  @param[out] ec          - out-parameter for error reporting.
        //
        ...
        //      `std::bad_alloc` may be thrown by both overloads if memory allocation fails.
        */
        key(open_or_create_tag, const key_path& path, 
    ...
    public:
        ...
        */
        ...
        /*!
        //  @return `true` if the key is open (i.e. `*this` identifies a registry key), `false` 
        //          otherwise.
        */
    ...
    public:
        ...
        /*! The function creates all missing keys in the specified path. If the key already exists,
        //  the function opens it. \n
        //  The calling process must have `access_rights::create_sub_key` access to the key identified
        //  by `*this`. The access rights the key was opened with does not affect the operation.
        //
        ...
        //
        //  @return 
        //      A pair consisting of a `key` identifying a registry key and a `bool` denoting whether
        //      the  key was created. \n
        //      if the subkey name is empty (i.e. `path.key_name().empty()`), the returned `key` object
        //      will identify the same registry key as `*this` does, except for the case when ... 
        //      TODO: write about redirection and views. \n
        //      The overload that takes `std::error_code&` parameter returns `std::make_pair(key(), false)` 
        //      on error.
        //
        //  @throw 
        //      The overload that does not take a `std::error_code&` parameter throws `registry_error` on underlying OS API
        ...
        */
        ...
        //  by `path` refer to the same registry key. */
        ...
        */
        ...
        /*! \brief 
        //  Checks whether the registry key identified by `*this` and the registry key identified
        //  by `key` refer to the same registry key. */
        ...
        */
        ...
        /*! The key must have been opened with the `access_rights::query_value` access right.
        //
        //  @param[in]  mask - a mask specifying which members of key_id are filled out and which 
        //                     aren't. The members which aren't filled out will be set to 
        //                     `static_cast<uint32_t>(-1)`, except for `last_write_time`, which 
        //                     default value is `key_time_type::min()`.
        //
        //  @param[out] ec   - out-parameter for error reporting.
        //
        //  @return 
        //      An instance of registry::key_info. \n
        //      The overload that takes `std::error_code&` parameter sets all members but
        //      `last_write_time` to `static_cast<uint32_t>(-1)` on error, `last_write_time` 
        //      is set to `key_time_type::min()`.
        //
        //  @throw 
        //      The overload that does not take a `std::error_code&` parameter throws `registry_error` on underlying OS API
        //      errors, constructed with the OS error code as the error code argument. \n
        ...
        */
        ...
        /*!
        //  @param[in]  path - an key path specifying the subkey that this function checks the 
        //                     existence of.
        //
        //  @param[out] ec   - out-parameter for error reporting.
        //
        ...
        */
        ...
        //      if the subkey name is empty (i.e. `path.key_name().empty()`), the returned `key` 
        //      object will identify the same registry key as `*this` does, except for the case when ... 
        //      TODO: write about redirection and views. \n
        //      The overload that takes `std::error_code&` parameter returns an default-constructed
        //      value on error.
        //
        //  @throw 
        //      The overload that does not take a `std::error_code&` parameter throws `registry_error` on underlying OS API
        ...
        */
        ...
        //  Retrieves the type and data for the specified value name associated with the registry 
        ...
        */
        ...
        /*! The key must have been opened with the `access_rights::enumerate_sub_keys` and 
        ...
        //
        //  @return 
        //      The number of keys that were deleted (which may be zero if `path` did not exist 
        //      to begin with). \n
        //      The overload that takes `std::error_code&` parameter returns 
    ...
    public:
        ...
        //  If an error occured, the associated registry key is closed, regardless of whether any 
        //  error is reported by exception or error code.
        //
        //  @post `!is_open()`.
        //  
        //  @param[out] ec - out-parameter for error reporting.`
    ...
    inline access_rights& operator&=(access_rights& lhs, access_rights rhs) noexcept
    {
        return lhs = lhs & rhs;
    }
    ...
    inline void swap(key& lhs, key& rhs) noexcept
    {
        lhs.swap(rhs);
    }
